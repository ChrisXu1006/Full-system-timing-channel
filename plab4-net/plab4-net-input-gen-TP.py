...
dataset_template_d0 = ( "init_net_msg_d0( {}'h{{:0>{}x}}, {}'h{{:0>{}x}}, " \
                   + "{}'h{{:0>{}x}}, {}'h{{:0>{}x}} );" ) \
...
dataset_template_d1 = ( "init_net_msg_d1( {}'h{{:0>{}x}}, {}'h{{:0>{}x}}, " \
                   + "{}'h{{:0>{}x}}, {}'h{{:0>{}x}} );" ) \
                  .format( srcdest_nbits, hexchars( srcdest_nbits ), \
                           srcdest_nbits, hexchars( srcdest_nbits ), \
                           opaque_nbits,  hexchars(  opaque_nbits ), \
...
def print_dataset( src, dest, opaque, payload ):
  ...
  for i in xrange(size):
    print dataset_template_d0.format( src[i], dest[i], opaque[i], payload[i] )
    print dataset_template_d1.format( src[i], dest[i], opaque[i], payload[i] )
